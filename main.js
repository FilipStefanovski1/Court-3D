...
const lineMat = new THREE.LineBasicMaterial({
  color:0xfff07a, transparent:true, opacity:.9,
...
new GLTFLoader().load('./basketball_court.glb',(g)=>{
  ...
  const sc = 28/Math.max(size.x,.0001) // scale to ~28m long
  court.scale.setScalar(sc)

  // find floor Y (kinda rough but fine)
  const rc=new THREE.Raycaster(), ys=[], down=new THREE.Vector3(0,-1,0)
...
let drawing=false, currLine=null, pathPoints=[]
...
function endLine(){
  ...
  if(pathPoints.length<2){
    scene.remove(currLine); currLine.geometry.dispose()
  }else{
    logStack.push({type:'stroke',obj:currLine})
    scribbles.push(currLine)
  }
  currLine=null; pathPoints=[]
}
...
function undoLast(){
  ...
  const last=logStack.pop(); if(!last) return
  if(last.type==='stroke'){
    scene.remove(last.obj); last.obj.geometry.dispose()
    const i=scribbles.indexOf(last.obj); if(i>=0) scribbles.splice(i,1)
  }else if(last.type==='add'){
    scene.remove(last.obj); markerMap.delete(last.number)
...
function loadPlay(name){
  ...
  d.players.forEach(p=>{
    ...
    scene.add(mk); markerMap.set(p.n,mk); allLabels.push(mk.userData.label)
  })
  d.lines.forEach(l=>{
    const pts=[], arr=l.points
    for(let i=0;i<arr.length;i+=3) pts.push(new THREE.Vector3(arr[i],arr[i+1],arr[i+2]))
    const line=new THREE.Line(new THREE.BufferGeometry().setFromPoints(pts), lineMat)
    line.renderOrder=999; scene.add(line); scribbles.push(line)
  })
  camDirty=true; alert('loaded '+name)
}
function updateList(){
  const sel=document.getElementById('playList'); if(!sel) return
  sel.innerHTML=''
  Object.keys(localStorage).filter(k=>k.startsWith('play_')).forEach(k=>{
    const o=document.createElement('option')
    o.value=o.textContent=k.slice(5); sel.appendChild(o)
  })
}
...
//keeps the player number circles the same size on screen   no matter how far the camera moves in 3D space
...
function sizeLabels(){ for(let i=0;i<allLabels.length;i++) fitLabel(allLabels[i],MARKER_PX) }
// loop keeps redrawing the scene   updating camera controls
